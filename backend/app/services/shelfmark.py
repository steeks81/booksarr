...
@dataclass
class CachedSeriesInfo:
    """Cached series info for a book."""
...
_series_cache: dict[str, CachedSeriesInfo] = {}
_series_cache_lock = asyncio.Lock()


def get_series_from_cache(provider: str, book_id: str) -> CachedSeriesInfo | None:
...
def set_series_in_cache(
    ...
    key = f"{provider}:{book_id}"
    _series_cache[key] = CachedSeriesInfo(
...
async def search(query: str, media_type: str = "ebook", series: str | None = None, author: str | None = None, title: str | None = None) -> ShelfmarkSearchResponse:
    """
    ...
    if not query and not series and not author and not title:
    ...
    # For series search: use field=series with query=series_name
    # For author search: use field=author with query=author_name
    # For title search: use field=title with query=title
    # For regular search: just query parameter (defaults to general search)
    # 
    ...
    for item in raw_results:
        ...
        # Series info
        # NOTE: SM's Hardcover search results don't include series info - it only comes
        # from get_book() which uses a different GraphQL query. Series data will be
        # available in bookInfo after releases load. Investigate if SM could include
        # series in search results, or if BA should do an extra book lookup on click.
...
async def enrich_series_stream(
    ...
    total = len(books)
    delayed_mode_until = 0.0  # timestamp when delayed mode expires
    DELAY_BETWEEN_REQUESTS = 0.4  # seconds
    COOLDOWN_AFTER_ERROR = 5.0  # seconds to stay in delayed mode after error
    RETRY_WAIT = 2.0  # seconds to wait before retrying failed request
    ...
    for i, book in enumerate(books):
        ...
        try:
            ...
            # Check for error (rate limit, timeout, etc.)
            if detail.error:
                logger.info("Series enrichment error for %s/%s: %s - retrying after %.1fs", 
                           provider, book_id, detail.error, RETRY_WAIT)
                await asyncio.sleep(RETRY_WAIT)
                detail = await get_book(provider, book_id)
                
                # Enter delayed mode for cooldown period
                delayed_mode_until = time.monotonic() + COOLDOWN_AFTER_ERROR
                logger.info("Entering delayed mode for %.1fs", COOLDOWN_AFTER_ERROR)
            ...
            logger.warning("Failed to fetch series for %s/%s: %s", provider, book_id, e)
    
    logger.info("Series enrichment complete: %d cache hits, %d cache misses", cache_hits, cache_misses)
    yield {"type": "done"}
...
async def get_book(provider: str, book_id: str) -> ShelfmarkBookDetailResponse:
    ...
    logger.info("Shelfmark book detail fetched: provider=%s book_id=%s title=%r description=%r series=%s source_url=%r", 
